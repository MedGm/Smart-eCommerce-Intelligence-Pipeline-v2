...
def run():
    ...
    if df["high_potential"].nunique() < 2:
        logger.warning("Target has a single class after construction. Skipping RF training.")
        return
    ...
    cv_threshold = optimize_f1_threshold(y, y_proba_cv)
    y_pred_cv = (y_proba_cv >= cv_threshold["best"]["threshold"]).astype(int)
    ...
    if use_mlflow:
        with mlflow.start_run(run_name="random_forest"):
            ...
            mlflow.sklearn.log_model(clf, "model", registered_model_name="rf_high_potential")
